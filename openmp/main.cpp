...
class Simplex{
    ...
    private:
        ...
        vector <vector<float> > A; // the matrix that stores the original data without kexi and w
        vector <vector<float> > Aw;  // the matrix that stores the auxiliary data with kexi and w
        int x0;
    ...
    public:
        Simplex(vector <vector<float> > matrix){
            maximum = 0;
            isUnbounded = false;
            lastPivotC_w = 0;
            printFreq = 1000;
            ...
            Aw.resize( rows + 1, vector<float>( cols + 1 , 0 ) );
            // B.resize(b.size());
            // C.resize(c.size());
        ...
        void ExchangeStep(vector <vector<float> > &matrix, int pivotRow, int pivotColume){
            ...
            matrix[pivotRow][pivotColume] = 1.0 / matrix[pivotRow][pivotColume];
            
            // update the pivot row
            #pragma omp parallel for
            for(int j = 0; j < cols; j++){
                if (j != pivotColume)
                    matrix[pivotRow][j] = -matrix[pivotRow][j] * matrix[pivotRow][pivotColume];
            }
            ...
            for(int i = 0; i < rows; i++)
                for(int j = 0; j < cols; j++){
                    if ((i != pivotRow) && (j != pivotColume))
                        matrix[i][j] = matrix[i][j] + matrix[i][pivotColume] * matrix[pivotRow][j];
            }
            ...
            for(int i = 0; i < rows;i++){
                if (i != pivotRow)
                    matrix[i][pivotColume] = matrix[i][pivotColume] * matrix[pivotRow][pivotColume];
            }
        ...
        void Reduce(){
            ...
            ExchangeStep(Aw, rows - 3, lastPivotC_w);
            for (int i = 0 ; i < rows ; i++)
                Aw[i][cols - 1] = Aw[i][cols - 1] + Aw[i][lastPivotC_w] * (-x0);

            // remove kexi column and w row
            for (int i = 0 ; i < rows - 1;i++){
                for (int j = 0 ; j < lastPivotC_w; j++)
                    A[i][j] = Aw[i][j];
                for (int j = lastPivotC_w + 1; j < cols; j++)
                    A[i][j - 1] = Aw[i][j];
            }
            cout << "Aw:" << endl;
            printM(Aw);
                    ...
                    if(rows < 1000000){
                        for(int l = 0; l < rows - 3; l++){
                            if (Aw[l][q] < 0)
                                if (p == -1)
                                    p = l;
                                else if((Aw[l][cols - 1] / Aw[l][q]) > (Aw[p][cols - 1] / Aw[p][q]))
                                    p = l;
                        }
                    }
                    else{
                        ...
                        #pragma omp parallel shared(max_local, max_idx)
                        {
                            ...
                            for(int l = start; l < stop; l++)
                                if (Aw[l][q] < 0)
                                    if ((Aw[l][cols - 1] / Aw[l][q]) > max_local[id])
                                    {
                                        max_local[id] = Aw[l][cols - 1] / Aw[l][q];
                                        max_idx[id] = l;
                                    }
			}
                ...
                if (!finished){
                    ...
                    for(int l = 0; l < rows - 1; l++){
                        if (A[l][q] < 0)
                            if (p == -1)
                                p = l;
                            else if((A[l][cols - 1] / A[l][q]) > (A[p][cols - 1] / A[p][q]))
                                p = l;
                    }
                    finished = (p == -1);
                    if (finished){
        ...
        void freeEliminate(int col){
            ...
            if (Aw[rows - 1][col] > 0){
                for (int i = 0; i < rows - 2; i++){
                    if (Aw[i][col] < 0){
                        float bk_it = Aw[i][cols - 1] / Aw[i][col];
                        if (bk_it < min){
                            min = bk_it;
                            chosen_row = i;
                        }
            ...
            ExchangeStep(Aw, chosen_row, col);
            printM(Aw);
            // change the position of kexi row
            if (chosen_row != rows - 3){
                vector<float> t(cols, 0);
                t = Aw[rows - 3];
                Aw[rows - 3] = Aw[chosen_row];
                Aw[chosen_row] = t;
            }
        }

        void printM(vector<vector<float> > matrix){
            int rows = matrix.size();
            int cols = matrix[0].size();
            if(rows > 30)
                return;
        ...
        void printZ(vector<vector<float> > matrix){
            int rows = matrix.size();
            int cols = matrix[0].size();
            for(int j= 0; j< cols;j++ ){
